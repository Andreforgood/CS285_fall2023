...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        action = agent.get_action(observation, epsilon) # ε-greedy via agent

        # TODO(student): Step the environment 与环境交互一步
        next_observation, reward, done, info = env.step(action)
        next_observation = np.asarray(next_observation)
        truncated = info.get("TimeLimit.truncated", False)
        done_for_training = done and not truncated
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            replay_buffer.insert(
                ...
                next_observation=next_observation[-1, ...],  # 取堆叠的最后一帧 (H, W)
                done=np.array(done_for_training),
            )
        else:
        ...
        if step >= config["learning_starts"]:
            ...
            update_info = agent.update(
                ...
                reward=batch["rewards"].squeeze(-1) if batch["rewards"].ndim > 1 else batch["rewards"],
                next_obs=batch["next_observations"],
                done=batch["dones"].float().squeeze(-1) if batch["dones"].ndim > 1 else batch["dones"].float(),
                step=step,
            )
